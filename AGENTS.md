...
| 版本 | versionName `1.0.49` / versionCode `50` | `defaultConfig` |
...
### ✅ 已完成
- **v1.0.49 修复 ASR 闪退 + 全量错误处理 + 发版**：用户反馈"填了远程模型信息，在对话里一按就闪退，离线 ASR功能也不能用"。根因分析:VoiceConversationController 中所有 lambda 调用(useRemoteAsr/remoteAsrApiKey/remoteAsrBaseUrl/asrEnginePref)和 beginListening/beginRemoteListening/beginSystemListening/beginSherpaListening 均无 try-catch,任何异常直接传播到 Compose 点击处理器导致闪退;RemoteTranscriber catch(Exception) 不 catch Throwable(Error 子类如 OOM/NoClassDefFoundError 会崩溃);VoiceRecorder.start() 同样 catch(Exception) 不 catch(Throwable)。3 文件修复:① **VoiceConversationController.kt**(287→352 行) — start() 包 try-catch,beginListening() 中每个 lambda 单独 try-catch+降级,beginRemoteListening 的 scope.launch 体 try-catch,所有 beginXxxListening 方法包 try-catch,全面 Log.i/w/e;② **RemoteTranscriber.kt**(61→80 行) — catch(Throwable),加 AppLog 日志(HTTP 状态/解析文本/错误体),验证音频文件存在;③ **VoiceRecorder.kt**(319→336 行) — start() catch(Throwable),加 android.util.Log,finishWithPcm 空 PCM 时调 onError(之前静默丢弃)。CI #31940646117 全绿,Build & Release #31940658037 全绿,Release `Agora-v1.0.49-android-arm64-v8a.apk` (22.7 MB) 已发布。
...
- 2026-08-16 v1.0.49 修复 ASR 闪退 + 全量错误处理 + 发版成功（本次会话）：用户反馈"填了远程模型信息，在对话里一按就闪退，离线 ASR功能也不能用"。根因分析:VoiceConversationController 中所有 lambda 调用(useRemoteAsr/remoteAsrApiKey/remoteAsrBaseUrl/asrEnginePref)和 beginListening/beginRemoteListening/beginSystemListening/beginSherpaListening 均无 try-catch,任何异常直接传播到 Compose 点击处理器导致闪退;RemoteTranscriber catch(Exception) 不 catch Throwable(Error 子类如 OOM/NoClassDefFoundError 会崩溃);VoiceRecorder.start() 同样 catch(Exception) 不 catch(Throwable)。3 文件修复:① **VoiceConversationController.kt**(287→352 行) — start() 包 try-catch,beginListening() 中每个 lambda 单独 try-catch+降级,beginRemoteListening 的 scope.launch 体 try-catch,所有 beginXxxListening 方法包 try-catch,全面 Log.i/w/e;② **RemoteTranscriber.kt**(61→80 行) — catch(Throwable),加 AppLog 日志(HTTP 状态/解析文本/错误体),验证音频文件存在;③ **VoiceRecorder.kt**(319→336 行) — start() catch(Throwable),加 android.util.Log,finishWithPcm 空 PCM 时调 onError(之前静默丢弃)。CI #31940646117 全绿,Build & Release #31940658037 全绿,Release `Agora-v1.0.49-android-arm64-v8a.apk` (22.7 MB) 已发布。
