...
- [x] ASR 集成（P1）：sherpa-onnx 端侧 ASR（OnlineRecognizer 流式 + OfflineRecognizer 批处理 + Silero VAD）+ 系统 SpeechRecognizer 在线回退 + 模型下载管理 + 设置 UI（v1.0.27 抽象层 + v1.0.29 设置 UI + v1.0.39 sherpa-onnx 原生库 + 模型下载 UI 接线）。**v1.0.39 待 push CI 验证**：sherpa-onnx .so 由 CI 下载（download-sherpa.sh），本地无法 push（GitHub HTTPS 443 不通）。
