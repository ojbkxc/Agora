...
| 版本 | versionName `1.0.41` / versionCode `42` | `defaultConfig` |
...
### ✅ 已完成
- **v1.0.41 sherpa-onnx 模型 URL/自动加载/tar.bz2 解压/watchdog 修复 + 发版**：用户反馈"语音识别到处都是问题"，深度分析发现更深层问题：TTS URL 错误（k2-fsa/kokoro-82M 不存在）、缺 espeak-ng-data 目录、模型不自动加载、ASR 用 float32 浪费体积。修复 6 文件：① SherpaModelManager 完全重写——TTS URL 改为 GitHub Releases tar.bz2（kokoro-multi-lang-v1_0.tar.bz2），新增 extractTarBz2()（Commons Compress），ASR 改用 int8 encoder/joiner，isModelPresent 完整检查 espeak-ng-data + lexicon + ruleFsts，ModelKind 加 sizeHint；② SherpaAsrEngine——init() 自动加载已下载模型，loadModel 优先 int8 文件名，numThreads 动态（1-4）；③ SherpaTtsEngine——init() 自动加载已下载模型，KOKORO 配置加 lexicon + ruleFsts 多语言支持；④ TtsManager——修 sherpa watchdog（子 mirrorJob + delay 超时替代阻塞 collect）；⑤ VoiceRecorder——VAD 路径改用 SherpaModelManager.modelDir，maxSpeech 8s→20s；⑥ SettingsSherpaModelsSection——显示模型大小提示（~2/~180/~200 MB）。CI #31896108969 全绿，Build & Release #31896423781 全绿（get-version ✓ / build-android 9m19s ✓ / release ✓），Release `Agora-v1.0.41-android-arm64-v8a.apk` 已发布。
