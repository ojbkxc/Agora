...
示例:
  ...
  # 仅检查 CI 状态
  python auto_fix.py --check-only

  # 指定仓库和迭代次数
  python auto_fix.py --repo ojbkxc/Agora --max-iterations 5

  # 使用 GitHub token (可自动推送)
  python auto_fix.py --github-token ghp_xxxxx
...
class Logger:
    def __init__(self, log_dir: str, verbose: bool = False):
        ...
        self.log_file = self.log_dir / f"auto_fix_{timestamp}.log"
    ...
    def log(self, msg: str, level: str = "INFO", force: bool = False):
        ...
        self._file.flush()
        if self.verbose or level in ("ERROR", "WARN", "SUCCESS") or force:
            print(line)

    def section(self, title: str):
        self.log("=" * 60, force=True)
        self.log(f"  {title}", force=True)
        self.log("=" * 60, force=True)
...
class GitHubActionsChecker:
    ...
    def get_latest_run_for_commit(self, commit_sha: str) -> Optional[Dict]:
        ...
        if data and "workflow_runs" in data and data["workflow_runs"]:
            # 返回最新的（非 queued/in_progress 的优先）
                ...
                if job_conclusion != "success":
                    ...
                    if self.token:
                        ...
                        if log_content:
                            ...
                            error_lines = [l for l in lines if any(kw in l.lower() for kw in ["error", "fail", "panic", "exception", "fatal"])]
                            if error_lines:
...
class HahaBunBackend(AIBackend):
    ...
    def fix(self, prompt: str, cwd: str) -> Tuple[bool, str]:
        ...
        if rc == 0 and output:
            ...
            return True, ""  # 空输出不代表失败，AI 可能直接修改了文件
...
class ChangeDetector:
    ...
    def get_changed_files(self, project_dir: str) -> List[Dict]:
        """
        检测已修改的文件（通过 git status）
        返回: [{"path": "relative/path", "content": "file content"}, ...]
        """
        project_path = Path(project_dir)

        # 尝试用 git status 检测
        changed_paths = self._git_changed_files(project_dir)

        if not changed_paths:
            self.logger.log("没有检测到文件变更", "INFO")
            return []

        self.logger.log(f"检测到 {len(changed_paths)} 个变更文件", "INFO")
    ...
    def _git_changed_files(self, cwd: str) -> List[str]:
        ...
        try:
            ...
            self.logger.log(f"git status 失败: {e}", "WARN")
...
class GitHubPusher:
    ...
    def push_files(self, files: List[Dict], commit_message: str) -> bool:
        ...
        # 使用 Git Database API 创建一次性提交
        # 1. 获取当前 commit 的 tree SHA
    ...
    def _api_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Optional[Any]:
        url = f"{self.API_BASE}/repos/{self.repo}/{endpoint}"
        headers = {
            "Accept": "application/vnd.github+json",
            "Authorization": f"Bearer {self.token}",
...
class AutoFixLoop:
    def __init__(self, config: Dict):
        self.config = config
        self.logger = Logger(
            config["log_dir"] or str(Path(config["project_dir"]) / "auto_fix_logs"),
            config["verbose"],
        )
    ...
    def _generate_prompt(self, iteration: int, error_info: str = "") -> str:
        ...
        if self.config["prompt"]:
            ...
            base = "fix the compilation errors"

        if iteration == 1 and error_info:
            truncated = error_info[-4000:] if len(error_info) > 4000 else error_info
            prompt = (
            ...
            prompt = (
                ...
                f"Here are the current errors:\n\n{truncated}\n\n"
                f"Please fix these errors carefully. "
                f"After fixing, summarize the changes you made."
            )
        else:
            prompt = f"Please analyze this project and {base}."
    ...
    def run(self) -> bool:
        ...
        for iteration in range(1, self.max_iterations + 1):
            ...
            if self.pusher.can_push() and not self.config["dry_run"]:
                ...
                self.logger.log(f"共 {len(changed_files)} 个文件:", force=True)
                for f in changed_files:
                    self.logger.log(f"  {f['path']}")

                if not self.config["dry_run"]:
                    self.logger.log("", force=True)
                    self.logger.log("请由 TRAE 通过 MCP push_files 推送以下内容:", "WARN", force=True)
                    self.logger.log(f"  owner: {self.config['repo'].split('/')[0]}", force=True)
                    self.logger.log(f"  repo: {self.config['repo'].split('/')[1]}", force=True)
                    self.logger.log(f"  branch: {self.config['branch']}", force=True)
                    self.logger.log(f"  message: {commit_msg}", force=True)
                    self.logger.log(f"  files: {json_path}", force=True)
                    self.logger.log("", force=True)
                    self.logger.log("推送后重新运行此脚本检查 CI 状态", "INFO", force=True)
                    return False  # 需要 TRAE 推送后重新运行
...
def main():
    ...
    # 打印日志文件位置
    log_dir = config["log_dir"] or str(Path(config["project_dir"]) / "auto_fix_logs")
    latest_logs = sorted(Path(log_dir).glob("auto_fix_*.log"), reverse=True)
    if latest_logs:
        print(f"\n日志文件: {latest_logs[0]}")
