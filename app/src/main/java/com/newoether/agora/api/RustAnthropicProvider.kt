 ...
 *
 * All HTTP transport and SSE parsing run inside the `agora_rs` native library.
...
open class RustAnthropicProvider : LlmProvider {
    ...
    override fun generateResponse(
        ...
        if (handle < 0) {
            trySend(StreamEvent.Error(GenerationError.Configuration("Rust Anthropic provider creation failed (code $handle)")))
            close()
            return@callbackFlow
        }
    ...
    companion object {
        private const val TAG = "RustAnthropicProvider"
    }
}
